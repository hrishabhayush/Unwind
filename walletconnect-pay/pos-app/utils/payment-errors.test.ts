...
describe("getPaymentErrorMessage", () => {
  describe("known error statuses", () => {
    ...
    it('returns failed message for "failed" status', () => {
      const result = getPaymentErrorMessage("failed");
      expect(result.title).toBe("Payment was declined or failed");
      expect(result.subtitle).toContain("payer wallet");
    });
  });
